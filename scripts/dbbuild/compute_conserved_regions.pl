...
=head1 SYNOPSIS
  ...
  compute_conserved_regions_filter_exons.pl
        ...
        -ud ucsc_db_name -ut ucsc_track_name
        [-s start] [-e end]
        [-fe] [-gc]
...
Arguments switches may be abbreviated where unique.
   ...
   -fe                  = filter all exons within the promoter search
                          region
   -gc                  = compute the gc content of the conserved regions
...
This is the oPOSSUM3 script for computing conserved regions at each
...
# XXX Only required if computing GC content XXX
...
if ($compute_gc) {
    pod2usage(
        -msg     => "To allow -gc option, edit and uncomment appropriate code",
        -verbose => 1
    );
}
...
foreach my $level (@$srl_levels) {
    ...
    $logger->info(
        sprintf("\t%d\t%d\t%d\n",
            $srl->level, $srl->upstream_bp, $srl->downstream_bp)
    );
}
...
#    || $logger->logdie("getting GeneAdaptor");
#my $ens_sa = $ens_db->get_SliceAdaptor
#    || $logger->logdie("getting SliceAdaptor");
...
foreach my $gid (@$gids) {
    ...
    #my $slice =
    #    $ens_sa->fetch_by_region('chromosome', $opgene->chr, $sr_start,
    #    $sr_end);
    ...
    my $search_regs =
        $opgene->promoter_search_regions($max_upstream_bp,
        $max_downstream_bp);
    ...
    if ($filter_all_exons) {
        $exons =
            $exa->fetch_by_region($opgene->chr, $opgene->start, $opgene->end);
    } else {
        $exons = $opgene->exons();
    }
    ...
    foreach my $clevel (@$cl_levels) {
        ...
        # XXX Need to pass search seq if computing GC content XXX
        #write_conserved_regions(
        #    \*OCRFH, $opgene->id, $clevel, $cons_regs, $search_seq
        #);
...
sub write_conserved_regions {
    ...
    foreach my $cr (@$crs) {
        ...
        #
        # XXX
        # If computing GC content, uncomment all the other code for
        # extracting sequence from Ensembl
        # XXX
        #
        #if ($compute_gc) {
        #    #
        ...
        #    my $reg_length = $cr_end - $cr_start + 1;
        #
        ...
        #    $logger->debug("$cr_seq");
        #
        #    $gc_content = calculate_gc_content($cr_seq);
