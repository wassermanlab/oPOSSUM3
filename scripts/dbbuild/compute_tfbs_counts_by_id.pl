...
use constant LOG_FILE  => 'compute_tfbs_counts_custom.log';

my $opossum_db_name = 'oPOSSUM_2010_worm';
...
if (!$tf_id) {
    pod2usage(
        -msg    => "Must provide either the TF ID to be searched",
        -verbose => 1
    );
}
    ...
    foreach my $cl_level (@$cl_levels) {
        ...
        foreach my $srl_level (@$srl_levels) {
            ...
            foreach my $thl_level (@$thl_levels) {
                ...
                my $tfbs_count = $ctfbsa->fetch_gene_tfbs_count(
                    -gene_id            => $gid,
                    -tf_id              => $tf_id,
