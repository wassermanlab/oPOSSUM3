...
$out_file = "$results_dir/opossum_gene_acsa.out" if !$out_file;

$log_file = "$results_dir/opossum_gene_acsa.log" if !$log_file;
...
unless ($anchor_matrix) {
    $logger->logdie("Error fetching anchoring TF profile from JASPAR DB");
}
...
sub write_hits_text
{
    ...
    #$text .= "\n\n";
    $text .= qq{\tChr\tStart\tEnd\tStrand\tNearest TSS\tAnchoring TF\tStart\tEnd\t};
    $text .= qq{Rel. Start\tRel. End\tStrand\tScore\t%%Score\tSequence\t};
    $text .= qq{Anchored TF\tStart\tEnd\tRel. Start\tRel. End\tStrand\tScore\t%%Score\tSequence\tDistance\n};
...
{
    my ($tf, $genes, $gid_sitepairs, $operon_genes) = @_;
    ...
    my $text .= sprintf("%s", $tf->name());

    #$text .= sprintf("\t%s", $tf->ID());
    #$text .= sprintf("\t%s", $tf->class() || 'N/A');
    #$text .= sprintf("\t%s", $tf->tag('family') || 'N/A');
    #$text .= sprintf("\t%s\n", $tf->tag('tax_group') || 'N/A');
    #$text .= sprintf("\t%s\n", $tf_total_ic);
    ...
    foreach my $gene (@$genes) {
        ...
        if (!$sitepairs or scalar @$sitepairs == 0) {
            $text .= "\t" x 20 . "\n";
            next;
        }
