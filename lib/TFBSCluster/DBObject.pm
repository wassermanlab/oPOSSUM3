...
package TFBSCluster::DBObject;

use TFBSCluster::Utils::Argument qw(rearrange);

=head2 new

  Arg [-ADAPTOR] : TFBSCluster::DBSQL::BaseAdaptor
  Arg [-db_id]    : database internal id
  Example        : none 
  Caller         : internal calls
  Description    : create a new DBObject object 
  Returntype     : TFBSCluster::DBObject
  Exceptions     : Adaptor not a TFBSCluster::DBSQL::BaseAdaptor
  Status         : Stable
...
sub new
{
    ...
    if ($adaptor) {
        if (   !ref($adaptor)
            || !$adaptor->isa('TFBSCluster::DBSQL::BaseAdaptor'))
        {
            throw(
                '-ADAPTOR argument must be a TFBSCluster::DBSQL::BaseAdaptor'
            );
        }
    }
...
=head2 adaptor
  ...
  Arg [1]    : TFBSCluster::DBSQL::BaseAdaptor $adaptor
  Example    : none
  Description: get/set for this objects Adaptor
  Returntype : TFBSCluster::DBSQL::BaseAdaptor
  Exceptions : none
  Caller     : general, set from adaptor on store
  Status     : Stable
...
sub adaptor
{
    ...
    if (@_) {
        my $ad = shift;
        if ($ad
            && (!ref($ad) || !$ad->isa('TFBSCluster::DBSQL::BaseAdaptor')))
        {
            throw(
                'Adaptor argument must be a TFBSCluster::DBSQL::BaseAdaptor'
            );
