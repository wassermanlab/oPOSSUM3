...
sub process
{
	...
	if (!$inter_binding_dist) {
		return $self->error("Must provide the inter-binding distance");
	}
	...
	# 
    my $command = OPOSSUM_SCRIPTS_PATH . "/opossum_gene_acsa.pl"
   		. " -j $job_id"
	    . " -tgf $t_gene_filename"
		. " -atfid $anchor_tf_id"
