...
my $log_file = "$log_dir/oPOSSUM_gene_acsa";
...
sub input
{
    ...
    my $vars = {
        ...
        var_template            => "input_gene_acsa.html"
    };
...
sub process
{
    ...
    if ($tf_select_criteria eq 'specific') {
        foreach my $tf_id ($q->param('tf_ids')) {
            push @tf_ids, $tf_id;
        }

        if (!@tf_ids) {
            return $self->error("No specific CORE TFs selected");
        }
    } elsif ($tf_select_criteria eq 'min_ic') {
        $min_ic = $q->param('min_ic');

        if (!defined $min_ic) {
            return $self->error("No minimum matrix IC specified");
        }

        if ($min_ic < $dflt_min_ic) {
            return $self->error(
                "Minimum matrix IC specified is less than absolute minimum"
                . " of $dflt_min_ic bits"
            );
        }
        ...
        if ($num_tax_groups > 1) {
            @tax_groups = $q->param("tax_groups");
            if (scalar @tax_groups == 0) {
                return $self->error("No tax groups selected");
            }
    ...
    my $command = "./opossum_gene_acsa.pl -j $job_id -d $tempdir -s $species"
        . " -g $t_gene_ids_file -db $tf_db -co $tf_collection -id $anchor_tf_id"
    ...
    my $vars = {
        ...
        email               => $email,
        var_template        => "analysis_summary_gene_acsa.html"
    };
