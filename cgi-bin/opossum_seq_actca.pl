...
factors (TFs)  limited by external (JASPAR) database name and information
content or taxonomic supergroup or all TFs in the oPOSSUM database.
...
use constant TITLE         =>
    'oPOSSUM Sequence-based Anchored Combination TFBS Cluster Analysis';
...
# Initialize logging
#
my $log_file = "$results_dir/opossum_seq_actca.log";
...
$logger->info("Connecting to JASPAR DB $tf_db");
#
...
$logger->info("Connecting to TFBSCluster DB $cl_db");
...
# this needs to be set no matter what--not really tf_select_criteria material
...
my ($t_cluster_seq_sites,
    $t_cluster_seq_sitepairs) = anchored_tf_cluster_set_search_seqs(
    $tf_cluster_set, $tf_set, $anchor_cluster, \%t_seq_id_seqs, $threshold
);

fatal("No TFBS clusters found in test sequences") unless $t_cluster_seq_sites;

$logger->info("Searching background sequences for TFBSs");
my ($bg_cluster_seq_sites) = anchored_tf_cluster_set_search_seqs(
    $tf_cluster_set, $tf_set, $anchor_cluster, \%bg_seq_id_seqs, $threshold
);

fatal("No anchored TFBS clusters found in control sequences") unless $bg_cluster_seq_sites;

$logger->info("Computing target sequence TFBS clsuter counts");
...
fatal("Error combining Fisher and z-score result_set") unless $cresult_set;
...
my $cresult_list = $cresult_set->get_list(%result_params);
...
if ($cresult_list && $cresult_list->[0]) {
    ...
    write_tfbs_cluster_details($t_cluster_seq_sitepairs, \%t_seq_id_display_ids);
}
...
#
# Read in seqs from the given fasta file
#
sub read_seqs
{
    my $file = shift;
...
{
    my ($tf_cluster_set, $tf_set, $anchor_cluster, $seq_id_seqs, $threshold) = @_;
...
sub write_results_html
{
    ...
    my $vars = {
        ...
        anchor_tf_name      => $anchor_matrix->name,
...
{
    my ($cluster_seq_sitepairs, $seq_id_display_ids) = @_;
...
{
    my ($filename, $cl, $seq_ids, $seq_id_display_ids, $seq_sitepairs) = @_;
...
sub write_tfbs_cluster_details_html
{
    ...
    my $section = sprintf "c%s - c%s Binding Site Combinations",
...
#
sub filter_overlapping_siteset
{
    my ($siteset) = @_;
...
sub send_email
{
    ...
    my $msg = "Your oPOSSUM sequence ACTCA results are available at\n\n";
    $msg .= "$results_url\n\n";
    ...
    printf SM "To: %s\n", $email;
    printf SM "From: %s\n", ADMIN_EMAIL;
    print SM "Subject: oPOSSUM Sequence ACTCA results $job_id\n\n";
    print SM "$msg" ;
...
#
# sites = listref of OPOSSUM::ConservedTFBS objects
#
sub logprint_sites
{
    my ($tag, $sites) = @_;
    ...
    unless ($sites && scalar @$sites > 0) {
        $logger->debug("No sites");
        return;
    }

    my @sorted_sites = sort {$a->start <=> $b->start} @$sites;

    for (my $i = 0; $i < scalar @sorted_sites; $i++) {
        my $site = $sorted_sites[$i];
        $logger->debug(
            sprintf("%7d %7d %7d %7s",
                $site->id(),
                $site->start(),
...
sub logprint_sitepairs
{
    ...
    unless ($sitepairs) {
        $logger->debug("No anchor/TFBS cluster site pairs");
        return;
    }
...
sub fatal
{
    ...
    my $msg = "oPOSSUM ACTCA analysis failed\n";
    $msg .= "\nJob ID: $job_id\n";
    $msg .= "\nError: $error\n";
